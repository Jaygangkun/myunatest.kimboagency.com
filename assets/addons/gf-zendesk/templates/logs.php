...
.tablenav .tablenav-pages a,.tablenav-pages-navspan{display:inline-block;min-width:17px;border:1px solid #ccc;padding:3px 5px 7px;background:#e5e5e5;font-size:16px;line-height:1;font-weight:400;text-align:center}
  ...
  <div class="vx_wrap"> 
  <div><img alt="<?php _e("Zendesk Feeds", 'gravity-forms-zendesk-crm') ?>" title="<?php _e("Zendesk Feeds", 'gravity-forms-zendesk-crm') ?>" src="<?php echo $this->get_base_url()?>images/zendesk-crm-logo.png?ver=1" style="float:left; margin:0 7px 10px 0;" height="46" />
  <h2 style="margin-bottom: 12px"><?php _e("Zendesk Log", 'gravity-forms-zendesk-crm'); ?></h2>
  <div class="clear"></div>
</div>
...
<form id="vx_form" class="crm_form" method="get"><div>
  ...
  <input type="text" placeholder="<?php _e('Search','gravity-forms-zendesk-crm') ?>" value="<?php echo $this->post('search') ?>" name="search" class="crm_input_inline">
  <?php
...
?>
  ...
  <option value=""><?php _e('All Objects','gravity-forms-zendesk-crm') ?></option>
  <?php    
  ...
  <option value=""><?php _e('All Status','gravity-forms-zendesk-crm') ?></option>
  <?php
  ...
  <option value=""><?php _e('All Times','gravity-forms-zendesk-crm') ?></option>
  <?php
  ...
  <input type="text" name="start_date" placeholder="<?php _e('From Date','gravity-forms-zendesk-crm') ?>From Date" value="<?php echo $this->post('start_date');?>" class="vxc_date crm_input_inline" style="width: 100px">
  <input type="text" class="vxc_date crm_input_inline" value="<?php echo $this->post('end_date');?>" placeholder="<?php _e('To Date','gravity-forms-zendesk-crm') ?>" name="end_date"  style="width: 100px">
  </span>
 
  <button type="submit" title="<?php _e('Search','gravity-forms-zendesk-crm') ?>" name="search" class="button-secondary button crm_input_inline"><i class="fa fa-search"></i> <?php _e('Search','gravity-forms-zendesk-crm') ?></button> 
  ...
  <button type="submit" class="button-secondary button crm_input_inline" title="<?php _e('Apply','gravity-forms-zendesk-crm') ?>" id="vx_apply_bulk"><i class="fa fa-check"></i> <?php _e('Apply','gravity-forms-zendesk-crm') ?></button>
  ...
  ?>
  <button type="button" name="tab_action" title="<?php _e('Export as CSV','gravity-forms-zendesk-crm') ?>" id="vx_export" class="button-secondary button crm_input_inline vx_left_10"><i class="fa fa-download"></i> <?php _e('Export as CSV','gravity-forms-zendesk-crm') ?></button> 
  <?php
  }
        if($this->post('log_id') !="" ){
  if(isset($data['feeds'][0]['entry_id']) && $data['feeds'][0]['entry_id']!=""){
     $entry_id=$data['feeds'][0]['entry_id'];
      ?>
  <a href="<?php echo   $log_link.'&entry_id='.$entry_id;?>" title="<?php echo sprintf(__('View Entry# %s Logs','gravity-forms-zendesk-crm'),$entry_id); ?>" class="button vx_left_10"><i class="fa fa-hand-o-right"></i> <?php echo sprintf(__('View Entry# %s Logs','gravity-forms-zendesk-crm'),$entry_id); ?></a><?php
  }}
 if($this->post('entry_id') !="" || $this->post('log_id') !=""){
          ?><a href="<?php echo $log_link;?>" title="<?php _e('View All Logs','gravity-forms-zendesk-crm') ?>" class="button vx_left_10"><i class="fa fa-external-link"></i> <?php _e('View All Logs','gravity-forms-zendesk-crm') ?></a>        
          <?php
      }
...
if($items>0){
  ?>
  <div class="tablenav-pages"> <span id="paging_header" class="displaying-num"><?php _e('Displaying','gravity-forms-zendesk-crm') ?> <span id="paging_range_min_header"><?php echo $data['min'] ?></span> - <span id="paging_range_max_header"><?php echo $data['max'] ?></span> of <span id="paging_total_header"><?php echo $data['items'] ?></span></span><?php echo $data['links'] ?></div>
 <?php
}
        ?>       
  </div>
  ...
  <th scope="col" class="manage-column vx_sort"  data-name="crm_id"><?php _e("Zendesk ID", 'gravity-forms-zendesk-crm') ?>
  <i class="fa fa-caret-<?php echo $crm_order ?> vx_sort_icon <?php echo $crm_class ?>"></i>                          
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="entry_id"><?php _e("Entry ID", 'gravity-forms-zendesk-crm') ?>
  <i class="fa fa-caret-<?php echo $entry_order ?> vx_sort_icon <?php echo $entry_class ?>"></i>                      
  </th>
  <th scope="col" class="manage-column"  data-name="object"><?php _e("Description", 'gravity-forms-zendesk-crm') ?>
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="time"><?php _e("Time", 'gravity-forms-zendesk-crm') ?>
  <i class="fa fa-caret-<?php echo $time_order ?> vx_sort_icon <?php echo $time_class ?>"></i>
  </th>
  <th style="width: 40px"><?php _e('Detail','gravity-forms-zendesk-crm') ?></th>
  </tr>
  </thead>
  ...
  <th><?php _e('Detail','gravity-forms-zendesk-crm') ?></th>  
  </tr>
  ...
  <td class="vx_check_col"><input type="checkbox" name="log_id[]" value="<?php echo $row['id'] ?>" class="crm_input_check"></td>
    <td class="vx_icon_col"><img src="<?php echo $base_url ?>images/<?php echo $row["status_img"] ?>.png" alt="<?php echo $row["status"] ? __("Active", 'gravity-forms-zendesk-crm') : __("Inactive", 'gravity-forms-zendesk-crm');?>" title="<?php echo $row['title'];?>" class="crm_status_img" /></td>
  <td class="column-name" style="width:40%"><p><?php echo $row['a_link'] ?></p></td>
    ...
    <td><i class="vx_icons vx_detail fa fa-th-list" title="<?php _e('Expand Details','gravity-forms-zendesk-crm') ?>"></i></td>  
  </tr>
  ...
  <tr>
    <td colspan="4" style="padding:20px;">
        <?php _e("No Record(s) Found", 'gravity-forms-zendesk-crm'); ?>
    </td>
  </tr>
  <?php
  }
  ...
  ?>
    <div class="crm_actions tablenav">
   <a id="vx_clear_logs" class="button" title="<?php _e('Clear Zendesk Log','gravity-forms-zendesk-crm') ?>" href="<?php echo wp_nonce_url(admin_url('admin.php?page='.$this->post('page')."&view=log&vx_tab_action_".$this->id."=clear_logs"),'vx_nonce','vx_nonce'); ?>"><?php _e('Clear Zendesk Log','gravity-forms-zendesk-crm') ?></a>
  <div class="tablenav-pages"> <span id="paging_header" class="displaying-num"><?php _e('Displaying','gravity-forms-zendesk-crm') ?> <span id="paging_range_min_header"><?php echo $data['min'] ?></span> - <span id="paging_range_max_header"><?php echo $data['max'] ?></span> of <span id="paging_total_header"><?php echo $data['items'] ?></span></span><?php echo $data['links'] ?></div>
    </div>
  <?php
  }
...
if($(".crm_input_check:checked").length == 0){ 
    alert('<?php _e('Please select at least one entry','gravity-forms-zendesk-crm') ?>');
    return false;
}
var action=sel.val();
 ...
 if(!confirm('<?php _e('Exporting more than 4 entries may take too long.\\n Are you sure you want to continue?','gravity-forms-zendesk-crm') ?>')){
  e.preventDefault();    
   ...
   $("#vx_clear_logs").click(function(e){
      if(!confirm('<?php _e('Zendesk Logs will be deleted permanently. Do you want to continue?','gravity-forms-zendesk-crm') ?>')){
          e.preventDefault();
      }  
  })    
  ...
  $(document).on("click",".vx_detail,.vx_close_detail",function(e){
    ...
    if(td.is(":visible"))
    {
       ...
       if(is_main){
    icon.attr('title','<?php _e('Expand Detail','gravity-forms-zendesk-crm'); ?>');    
    }        
     return;   
    }else{
    next_tr.show('fast'); 
    ...
    if(is_main){
    icon.attr('title','<?php _e('Collapse Detail','gravity-forms-zendesk-crm'); ?>');    
    }    
    }
